...
def _xem_refresh(tvdb_id):
    ...
    try:
        logger.log(u'Looking up XEM scene mapping for show %s' % (tvdb_id,), logger.DEBUG)
        ...
        if data is None or data == '':
            logger.log(u'No XEN data for show "%s", trying TVTumbler' % (tvdb_id,), logger.MESSAGE)
            data = getURL('http://show-api.tvtumbler.com/api/thexem/all?id=%s&origin=tvdb&destination=scene' % (tvdb_id,))
            if data is None or data == '':
                logger.log(u'TVTumbler also failed for show "%s".  giving up.' % (tvdb_id,), logger.MESSAGE)
                return None
        ...
        if result:
            ...
            if result['result'] == 'success':
                ...
                #logger.log(u'Found XEM scene data for show %s' % (tvdb_id), logger.MESSAGE)
            else:
                logger.log(u'Failed to get XEM scene data for show %s because "%s"' % (tvdb_id, result['message']), logger.MESSAGE)
        else:
...
def get_xem_numbering_for_season(tvdb_id, season):
    """
    Returns a dict of (season, episode) : (sceneSeason, sceneEpisode) mappings
    for an entire show.  Both the keys and values of the dict are tuples.
    Will be empty if there are no scene numbers set
    ...
    rows = cacheDB.select('''SELECT season, scene_season
                        FROM xem_numbering WHERE tvdb_id = ? AND season = ?
                        ORDER BY season''', [tvdb_id, season])
    result = {}
    if rows:
        for row in rows:
