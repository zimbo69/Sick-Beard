...
def revertEpisode(show_obj, season, episode=None):
    ...
    sql_results = myDB.select("SELECT * FROM history WHERE showid=? AND season=?", [show_obj.tvdbid, season])
    # {episode: result, ...}
...
def findRelease(show, season, episode):
    ...
    myDB.action("DELETE FROM history WHERE showid=" + str(show.tvdbid) + " AND season=" + str(season) + " AND episode=" + str(episode) + " AND date < (SELECT max(date) FROM history WHERE showid=" + str(show.tvdbid) + " AND season=" + str(season) + " AND episode=" + str(episode) + ")")

    # Search for release in snatch history
    results = myDB.select("SELECT release, provider, date FROM history WHERE showid=? AND season=? AND episode=?",[show.tvdbid, season, episode])
